 ...
 */
		...
		static public bool Be(
			...
			,
			nilnul.fs.git.module.repo._branch.Name branchOther
			,
			...
			return nilnul.txt.nulable.be_.Nul0White.Singleton.be(
				nilnul.win.prog_.git.run.result.msg._TrimmedX.Txt(module,$"diff --name-only {branch}..{branchOther} --",git) /*replace ".." with " ", we will get the same*/
			); //name-status
		}
