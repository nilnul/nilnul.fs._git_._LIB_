...
namespace nilnul.fs.git.depo.repo.refers
{
	...
	///	
	/// <summary>
	/// Any time the the tip of branches are changed it's recorded in the reflog.This means that if you accidentally "lose" some data, there's a very good chance it is in the reflog, and you can retrieve it from there.
	/// </summary>
	/// When a commit is modified with git commit --amend or git rebase, it is not deleted, only replaced with a new commit with a new identity. We can always retrieve commits that are no longer visible via the reflog.
	///
	/// 
	///alembic.com.au/blog/the-identity-of-a-git-commit-explained
	/// ,by: Kathryn Prestridge
	///
	/// vs:
	///		<see cref="depo.repo.ILog"/>
	internal class ILog
	{
