...
Works like git push <remote>, where <remote> is the current branch’s remote (or origin, if no remote is configured for the current branch).
		...
		static public string PushAllBranches_retAll( nilnul.fs.Folder workingDir, string remote, nilnul.win.prog_.Git git=null)
		{
			return (git??Git.StaticInstance).runCmd__returnResult(workingDir, "push --all " + remote).ToString();    //all here means all branches.
		...
		/// <returns></returns>
		static public string PushAllBranchesByForce_retErr(nilnul.win.prog_.Git git, nilnul.fs.Folder workingDir, string remote)
		{
			var r =
		...
		static public string PushAllBranchesByForce_retErr(nilnul.fs.Folder workingDir, string remote,Git git = null)
		{
			return (git??Git.StaticInstance).runCmd__returnResult(workingDir, "push --all -f " + remote).ToString();  //all here means all branches.

		}

		static public string PushAllBranchesByForce_retErr( nilnul.fs.Folder workingDir, string remote)
		{
			return PushAllBranchesByForce_retErr(nilnul.win.prog_.Git.StaticInstance, workingDir,remote);
		...
		static public string PushAllBranches_retAll(nilnul.fs.Folder folder, string remote)
		{
...
Find a ref that matches master in the source repository (most likely, it would find refs/heads/master), and update the same ref (e.g. refs/heads/master) in origin repository with it. If master did not exist remotely, it would be created.
			 ...
			 */
		static public void RemoteBranch(nilnul.fs.git.Folder folder, string remote, string branch)
		{

			folder.runCmd_throwErr($"push {remote} {branch}");
		}
...
Commits A and B would no longer belong to a branch with a symbolic name, and so would be unreachable. As such, these commits would be removed by a git gc command on the origin repository.
		...
		static public void PushAllBranches(Folder folderGit, string remote)
		{

			folderGit.git.runCmd_throwErr(folderGit.address.ToString(), "push --all " + remote);    //all here means all branches.
