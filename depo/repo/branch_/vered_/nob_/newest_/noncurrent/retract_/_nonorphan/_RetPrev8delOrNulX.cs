...
namespace nilnul.fs.git.depo.repo.branch_.vered_.nob_.newest_.noncurrent.retract_._nonorphan
{
	/// <summary>
	/// if current is the branch to drop, this would fail;
	/// </summary>
	/// 
	public static class _RetPrev8delOrNulX
	{
		...
		{
			txt_.Vered[] sorted = fs.git.module.repo.branch_.vereds._SortX.Seq(module, nub4branch, git).ToArray();
			...
			if (count >= 2)
			{
				...
				if (
					nilnul.fs.git.module.repo.branch.co.be_._DestinationsSameX.Be(module, last, secondLast)
										&&
					nilnul.fs.git.module.repo.branch.history.snaps.be_._PluralX._IsPlural_1branch(module, secondLast.ToString(), git)
