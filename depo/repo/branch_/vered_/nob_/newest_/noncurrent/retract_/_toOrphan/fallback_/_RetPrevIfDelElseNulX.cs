...
{
	/// <summary>
	/// the newest branch is not current branch. let's say whether it can be regressed to an earlier branch if that is an orphan branch;
	/// </summary>
	/// <remarks>
			...
			if (count >= 2)
			{
				...
				if (
					nilnul.fs.git.module.repo.branch.co.be_._DestinationsSameX.Be(module, last, secondLast)
										&&
					nilnul.fs.git.depo.repo.branch.history.nodes.be_._SingularX._Is_1branch(module, secondLast.ToString(), git)
