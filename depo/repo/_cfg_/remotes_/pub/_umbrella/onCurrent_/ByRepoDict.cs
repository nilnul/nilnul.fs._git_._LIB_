...
namespace nilnul.fs.git.module.repo._cfg_.remotes_.pub._umbrella.onCurrent_
{
		...
		>
	{
		...
		public ByRepoDict(Dictionary<
				string
				,
				fs.git.svr.client.RepoI
			> val, nilnul.win.prog_.Git git0 = null) : base(val)
		{
			_git = git0;
		}
		...
		{
			var schema = fs.git.depo.repo.branch_._pub.Settings1.Schema;
			...
			for (int i = 0; i < this.boxed.Count(); i++)
			{
				...
				var noms = this.boxed.ElementAt(i).Key.Split('-');

				///get all branches
				///

				///for each host such as github
				var remotes4svc = remotes.Where(

					b =>
				nilnul.objs.re_.init_.EqDefault<string, nilnul.txt.eq_.CaseInsensitive>.Singleton.re(
					noms.Select(schemaSeg => nilnul.txt_.vered._KeyX._Txt(schemaSeg))
					,
