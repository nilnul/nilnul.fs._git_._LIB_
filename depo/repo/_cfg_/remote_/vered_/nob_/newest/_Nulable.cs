...
namespace nilnul.fs.git.module.repo._cfg_.remote_.vered_.name_.newest
{
	static public class _NulableX
	{
		...
		public static string _Txt_0depo_1nob(
			string folder
			,
			string name
			,
		...
		)
		{
			return
				nilnul.txt.aggregate_.VerLatestOrNul.Singleton.aggregate(
					vered_.name._VwX._Txts_0depo_1nob(folder,name,git) 
				)
			;
		}
		public static string _Txt_0depo(
			string folder
			,
		...
		)
		{
			return
				nilnul.txt.aggregate_.VerLatestOrNul.Singleton.aggregate(
					vered_.name._VwX._Txts_0depo(folder,name,git) 
				)
			;
		}
		...
		)
		{
			return
				nilnul.txt.aggregate_.VerLatestOrNul.Singleton.aggregate(
					vered_.name._VwX.Exe(folder,name,git) 
				)
			;
		}
