...
namespace nilnul.fs.git.module.repo._cfg_.remote_.vered_.name
{
	...
	{
		public static IEnumerable<string> _Txts_0depo_1nob(
			string folder, 
			string name, 
			nilnul.win.prog_.Git git =null
		)
		{
			return remotes._VwX._Seq_0depo(folder,git).Where(
				x=> nilnul.txt_.vered.be_._NamedAsX.Be(name,x)
			);
		}
		public static IEnumerable<string> _Txts_0depo(
			string folder, 
		...
		{
			return remotes._VwX.Seq(folder,git).Where(
				x=> nilnul.txt_.vered.be_._NamedAsX.Be(name,x)
			);
		}
