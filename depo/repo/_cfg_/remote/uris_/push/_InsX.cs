...
namespace nilnul.fs.git.depo.repo._cfg_.remote.uris_.push
{
	static public class _InsertX
	{
		/*
...
git remote set-url origin --push --add user2@repo2
			 ...
			 */
		static public void _Vod_0depo_1remote_2uri(string folder, string remote, string url, nilnul.win.prog_.Git git=null)
		{
			var argument = $"remote set-url --add {remote} {url}";
			try
			{
				nilnul.win.prog_.git.run.cod.vow_._NilX._Vod_0address_1argument(folder,argument,git);
		...
		static public void _Vod_0depo_2uri(string folder, module.repo._cfg_._remote.Name remote, string url, nilnul.win.prog_.Git git=null)
		{
			 _Vod_0depo_1remote_2uri(folder, remote, url, git);
			return;
		

		}

		static public void _Vod_urnAssumeValid(nilnul.fs.git.ModuleI folder, module.repo._cfg_._remote.Name remote, string url, nilnul.win.prog_.Git git=null)
		{
			 _Vod_0depo_2uri(folder.ToString(),remote,url,git);
			return;
		...
		static public void _Vod_urnAssumeValid(nilnul.fs.git.Module folder, module.repo._cfg_._remote.Name remote, string url, nilnul.win.prog_.Git git=null)
		{
			_Vod_urnAssumeValid(folder as nilnul.fs.git.ModuleI,remote,url,git);

		}
		static public void _Vod_urnAssumeValid(nilnul.fs.folder_.git_.Top folder, module.repo._cfg_._remote.Name remote, string url, nilnul.win.prog_.Git git=null)
		{
		...
		{
			_Vod_urnAssumeValid(new fs.folder_.git_.Top( folder), new module.repo._cfg_._remote.Name( remote), (url), git);
		}
		...
		{
			_Vod_urnAssumeValid( fs.folder_.git_.Top.FroAddress( folder), new module.repo._cfg_._remote.Name( remote), (url), git);
		}
		...
		{
			_Vod_urnAssumeValid(gitTop, new module.repo._cfg_._remote.Name(remoteKey), (urn),git);
		}
