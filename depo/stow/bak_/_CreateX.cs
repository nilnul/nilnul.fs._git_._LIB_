...
namespace nilnul.fs.git.module.stow
{
	...
	/// simulate git stash; we don't use git Stash as it errors if what to stash is empty or if the head commit is unborn.
	/// </summary>
	/// <remarks>
	/// the old head would be restored at the end;
	/// </remarks>
	///
	[Obsolete(nameof(depo.stow.bak_._WerkAtOrphanX))]
		...
		)
		{
			...
			try
			{
				new nilnul.obj.vow_.true_.xpn_.Unacceptable(
					...
					nilnul.fs.git.module.index.add_.all_.minWinTitled.commit_.allowEmpty_._MinWinTitledX.CommitExitCode_AddAndCommit(
						_location
						,
						$"add and commit all for backup" //don't change this as we need to keep the commit intact given same treeish;
					, git) == 0
				);
			}
			catch (Exception e)
			...
			#region make a tmp branch such that we can step back with the tip therein moving, without affecting others
			...
			nilnul.win.prog_.git.run.exitCode.vow_._NilX.OfAddress(_location, $"checkout -b {tempBranch}", $"exception when checkout -b {tempBranch} @{_location}", git); // as the below operation might move current branch, hence we create new one without worrying about moving other branch;
			...
			catch (Exception x)
			{
				...
				throw new fs.git.repo.act_.xpn_.FsAccessDenied(
					$"deleting branch:{tempBranch} at {_location} failed, cuz that, say, some file is already occupied as explained in: could not delete reference refs/heads/_2301061150396684925: unable to create file D:/13/d/someDepo1/.git/packed-refs.new: File exists;", x
				);
			}
