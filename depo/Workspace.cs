...
namespace nilnul.fs.git.module
{
	...
	///		subfolder of root that's the root of another sub-model inside this model.
	///		
	/// </summary>
	/// 
	public class Workspace
		:
		nilnul.fs.folder.categorized.En<git.folder.categorized.be_.Work>
		,WorkspaceI
		//nilnul.be.Asserted< nilnul.dev.git.Folder, nilnul.dev.git.folder.be_.Inited>
	{

		public Workspace(git.folder.Categorized c):base(c)
		{

		}
		private Workspace(Folder folder):this( git.folder.Categorized.Eval(folder))
		{
		...
		public string getRoot()
		{
			...
			var result = this.avowed.folder.runCmd( "rev-parse --show-toplevel");
			if (result.notBeErr)
			{
...
And then you'd be able to run git root to get the root of your current tree. Note that in the shell script, use "-e" to cause the shell to exit if the rev-parse fails so that you can properly get the exit status and error message if you are not in a git directory.
		...
		*/
		...
		public bool isRoot()
		{
			return getRoot() == new DirectoryInfo(this.avowed.folder.address.ToString()).ToString();
