...
new nilnul.fs.git.svr_.azure.Client(org, token)
 ...
 nilnul.fs.git.svr_.azure.Client.CreateFroVault()
		...
		public Dictionary<TeamProjectReference, IEnumerable<WebApiTeam>> ListAllProjectsAndTeams()
		{
			...
			IEnumerable<TeamProjectReference> projects = projectClient.GetProjects().Result; /// todo: consider to use GetAwaiter().GetResult(); ditto for what follows;
		...
		public TeamProjectReference GetProjectDetails(string projectName)
		{
			...
			Trace.WriteLine("Details for project {0}:", projectName);
			Trace.WriteLine("");
			Trace.WriteLine($"  ID          : {project.Id}" );
			Trace.WriteLine($"  Description : {project.Description}" );
			Trace.WriteLine($"  Web URL     : {(webLink != null ? webLink.Href : "(not available)")}" );
...
"Project creation operation failed: " + completedOperation.ResultMessage
			...
			CancellationToken cancellationToken = default(CancellationToken))
		{
			...
			while (true)
			{
				//Trace.WriteLine(" Checking status ({0})... ", (checkCount++));

				Operation operation = await operationsClient.GetOperation(operationId, cancellationToken);

				if (!operation.Completed)
				{
					//Trace.WriteLine("   Pausing {0} seconds", interavalInSec);
		...
		public bool ChangeProjectDescription()
		{
			...
			//{
			//	Trace.WriteLine("No previously created project to change the description of.");
			//	return false;
			//}
			...
			Trace.WriteLine("Queuing project update...");
			...
			{
				Trace.WriteLine("");
				Trace.WriteLine($"Project:{project.Name} description change from:\n {project.Description }\nto\n {updatedProject.Description}" );
				return true;
			}
			else
			{
				Trace.WriteLine("");
				Trace.WriteLine("Unable to change the description for project {0}", project.Name);
				return false;
			}
		}
		...
		public bool RenameProject()
		{
			...
			//if (!Context.TryGetValue<TeamProject>("$newProject", out project))
			//{
			//	Trace.WriteLine("No previously created project to change the name of.");
			...
			{
				Trace.WriteLine("");
				Trace.WriteLine($"Project renamed from:\n {project.Name}\nto\n {updatedProject.Name}" );
				return true;
			}
			else
			{
		...
		//[ClientSampleMethod]
		bool DeleteProject(TeamProject project)
		{
			// Use the project created in the earlier "create project" sample method
			//TeamProject
			...
			//if (!Context.TryGetValue<TeamProject>("$newProject", out project))
			//{
			//	Trace.WriteLine("No previously created project found to delete.");
			...
			Trace.WriteLine("Queuing project delete...");
			...
			Trace.WriteLine("");
			Trace.WriteLine($"Delete project operation completed {operationResult.Status}" );
		...
		public IEnumerable<TeamProjectReference> ListProjectsByState()
		{
			...
			{
				Trace.WriteLine($"No projects with the state {state}" );
			}
