...
{
	//extern alias fs;
	static public class _DropX1
	{
		public static void ByRemote(nilnul.fs.git.ModuleI folder, _cfg_._remote.Name remote, nilnul.win.prog_.Git git=null)
		{
			nilnul.win.prog_.git.run.exitCode.vow_._NilX.Void(folder,  $"remote remove {remote}",git);
...
public static void ByRemote( string folder, string remote, nilnul.win.prog_.Git git =null)
		{
			ByRemote(  nilnul.fs.folder_.git_.Top.FroAddress(folder) ,remote, git);
