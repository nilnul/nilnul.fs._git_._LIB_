...
The size-pack value is the size of your repository when it is pushed to a remote server like Bitbucket. The size-pack value is in kilobytes.  So, in the above example the repository is not even 1 MB.  
		...
		public static win.prog._run.Result Result(nilnul.fs.git.ModuleI module, nilnul.win.prog_.Git git = null, int? life = null)
		{
			return nilnul.win.prog_.git.run_.timeout._ResultX.Result(module, "count-objects -vH", git, life ?? milisecondsNeeded);
