namespace nilnul.fs.git.module.repo.gc_.now_
{
	public static class _ByExpireUnreachableX
	{
		/*
...
Attention: Only using git gc --prune=now will not work since those commits are still referenced in the reflog. Therefore, clearing the reflog is mandatory.*/
		...
		)
		{
			return nilnul.txt.op_.binary_.join_.DuoLineBreak.Singleton.op(
				...
				nilnul.win.prog_.git.run.result.msg._TrimmedX.Txt(module, "reflog expire --expire-unreachable=all --all", git)
				,
				//nilnul.win.program_.Git.RunCmd__retMsg_throwErr(module, "gc --prune=now", git)
