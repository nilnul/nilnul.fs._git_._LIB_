...
namespace nilnul.fs.folder_.nonlink_.git_.work_.nontop.git_.modules_.nonlink_
{
	/// <summary>
	...
	public static class _InternalX
	{
		...
		{
			foreach (var internalDirectly in interior_._DirectlyX._Directories(_top,git))
			{
				var shield=   nilnul.fs.folder_._ParentDivisionX1.CreateFolder(_top, internalDirectly);
				foreach (
					var modulesContainedInInternalModules in nilnul.fs.folder_.nonlink_.git_.work_.top.git_.modules_.nonlink_._InsideX._Divs(shield, git)
				)
				{
