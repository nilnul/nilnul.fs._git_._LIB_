...
namespace nilnul.fs.folder_.git_
{
	...
	public class Work
		:
		nilnul.fs.folder.vow_.git_.work.En
	{
		...
		{
			return new Work(
					nilnul.fs.Folder1.CreateFroAddress(obj)
				);
		}
		static public Work FroAddress(string address)
		{
			return new Work(
				nilnul.fs.Folder1.CreateFroAddress(address)
			);
		}


		[Obsolete()]
		public nilnul.fs.address_.Shield getRoot()
		{

			///note: in ".git", the following command returns empty. 
			///		in submodule, the following command returns the root of the submodule.

			var result = Git.StaticInstance.runCmd__returnResult(this.ed.container.ToString(), "rev-parse --show-toplevel");
			if (result.notBeErr)
			{
		...
		public nilnul.fs.address_.ShieldI getRoot1(nilnul.win.prog_.Git git = null)
		{

			///note: in ".git", the following command returns empty. 
			///		in submodule, the following command returns the root of the submodule.

			var result = nilnul.win.prog_.Git.RunCmd_result(this.ed.container.ToString(), "rev-parse --show-toplevel");
			if (result.notBeErr)
			{
				var root = result.msg.ToString().Trim();    //note backslash and slash	//.Split(new[] { '\n', '\r' },StringSplitOptions.RemoveEmptyEntries);
				return nilnul.fs.address_.Shield.FroAddress(root);
		...
		static public nilnul.fs.address_.Shield _GetRoot(Git git, string _insideWork_notRepo)
		{
...
And then you'd be able to run git root to get the root of your current tree. Note that in the shell script, use "-e" to cause the shell to exit if the rev-parse fails so that you can properly get the exit status and error message if you are not in a git directory.
		...
		*/
		...
		public bool isRoot()
		{
			return nilnul.fs.address_.container.Eq1.Singleton.Equals(getRoot(), ed.container);
		...
		/// <returns></returns>
		static public bool _IsRoot(Git git, string _locationOf_folderInModule_NotRepo)
		{
			return nilnul.fs.address_.container.Eq.Singleton.Equals(
		...
		{
			return _IsRoot(Git.StaticInstance, _locationOf_folderInModule_NotRepo);
