...
namespace nilnul.os.prog_.git.run_.exit
{
	...
	{
		static public proc_.finished._result.CodMsgErr _Result_0address_1argument(
			string shield
			,
			string arg
			,
		...
		{
			return nilnul.os.prog.run_.exit.Result.Singleton.ofAddress
				(
		...
		static public proc_.finished._result.CodMsgErr _Result_0address(
			string shield
			,
			nilnul.win.prog._run.Args1  arg
			,
			nilnul.win.prog_.Git git = null
		)
		{
			return _Result_0address_1argument(shield,arg.ToString(),git);
		
		}
		static public proc_.finished._result.CodMsgErr _Result_0address(
			string shield
			,
			IEnumerable<Arg1> arg
			,
			nilnul.win.prog_.Git git = null
		)
		{
			return _Result_0address(shield, new Args1(arg),git);
		...
		{
			return _Result_0address(shield, new Args1(arg));
		
		}

		static public proc_.finished._result.CodMsgErr _Result_0address_1args(
			string shield
			,
			params string[] arg
		)
		{
			return _Result_0address(shield,arg.Select(a=> new Arg1(a)));
		}
		...
		{
			return _Result_0address(shield,arg.Select(a=> new Arg1(a)),git);
		}
		...
		)
		{
			return  //nilnul.win.proc_.started_.end.result_.StdSyn.Singleton.ofAddress
				nilnul.os.prog.run_.exit.Result.Singleton.ofAddress
				(
				nilnul.win.prog_.git.nulable.Fallback.NotNul(git).val.location
				, 
				arg
